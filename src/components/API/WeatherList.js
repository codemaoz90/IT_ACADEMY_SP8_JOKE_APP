...
function WeatherList(){
    ...
    const[weatherLdn, setWeatherLdn]= useState([])
     ...
     useEffect(() => {
        const fetchData = async() => { 
            const result = await 
                axios.get('https://api.weatherapi.com/v1/current.json?key=e3c4a31ebc6447ef8c0115758211003&q=London&aqi=no', {
                    headers: { 
                        'Accept':'application/json'
                    }
                });
    ...
    const[weatherNyc, setWeatherNyc]= useState([])
     ...
     useEffect(() => {
        const fetchData = async() => { 
            const result = await 
                axios.get('https://api.weatherapi.com/v1/current.json?key=e3c4a31ebc6447ef8c0115758211003&q=new york&aqi=no', {
                    headers: { 
                        'Accept':'application/json'
                    }
                });
    ...
    useEffect(() => {
        const fetchData = async() => { 
            const result = await 
                axios.get('https://api.weatherapi.com/v1/current.json?key=e3c4a31ebc6447ef8c0115758211003&q=moscow&aqi=no', {
                    headers: { 
                        'Accept':'application/json'
                    }
                });
